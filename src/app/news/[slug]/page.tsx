...
export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> {
  ...
  const cardImageUrl = `${siteUrl}/api/og?slug=${article.slug}&v=5`;
...
export default async function ArticleDetailPage({ params }: ArticlePageProps) {
  ...
  // 서버 내부 렌더링 맞춤형 타이포그래피 카드뉴스 URL (v=5 캐시 갱신으로 2026 보존 card_title 즉시 반영)
  const cardImageUrl = `/api/og?slug=${article.slug}&v=5`;
  ...
  const jsonLd = {
    ...
    image: [`${siteUrl}/api/og?slug=${article.slug}&v=5`],
