...
export function cleanseHeadline(title: string): string {
  if (!title) return "";
  let clean = title
    // [심층 분석], [긴급 점검], [속보], [단독], [김희철칼럼], [사설], [오피니언] 등 고정 말머리 말뚝 태그 제거
    .replace(/^\[(?:심층\s*분석|긴급\s*점검|속보|단독|기획|종합|포토|단독보도|특징주|해설|[가-힣a-zA-Z0-9\s]*칼럼|사설|오피니언|시론|기고|기자수첩|취재수첩)\]\s*/i, "")
    .replace(/^【(?:심층\s*분석|긴급\s*점검|속보|단독|기획|종합|[가-힣a-zA-Z0-9\s]*칼럼|사설|오피니언|시론|기고)】\s*/i, "")
    .replace(/^\((?:심층\s*분석|긴급\s*점검|속보|단독|[가-힣a-zA-Z0-9\s]*칼럼|사설|오피니언|시론|기고)\)\s*/i, "")
    .replace(/^(?:칼럼|사설|오피니언|시론|기고)\s*[:：]\s*/i, "")
...
export function repairMismatchedHeadings(
  ...
  const policyDummyPattern = /누가 받을 수 있나\?|지원 대상 및 자격 요건|신청 방법 및 향후 일정|본 제도는 지원이 절실한 실수요자/i;
  if (!isPolicyCategory && policyDummyPattern.test(result)) {
    wasRepaired = true;

    if (category === "금융·경제") {
      result = result
        .replace(/## (?:1\.\s*)?핵심 개요 및 주요 쟁점/, `## 1. 주요 동향 및 배경`)
        .replace(/## (?:2\.\s*)?(?:지원 대상 및 자격 요건|누가 받을 수 있나\?)/, `## 2. 기술·시장적 파급 효과`)
        .replace(/## (?:3\.\s*)?(?:세부 혜택 및 수치 비교|무엇이 얼마나 달라지나\?)/, `## 3. 핵심 지표 및 스펙 비교`)
        .replace(/## (?:4\.\s*)?(?:신청 방법 및 향후 일정|어떻게 신청하나\?)/, `## 4. 향후 전망 및 시사점`);
      ...
      // 사회·문화 및 정치/일반 시사
      result = result
        .replace(/## (?:1\.\s*)?핵심 개요 및 주요 쟁점/, `## 1. 핵심 개요 및 배경`)
        .replace(/## (?:2\.\s*)?(?:지원 대상 및 자격 요건|누가 받을 수 있나\?)/, `## 2. 주요 쟁점 및 파장`)
        .replace(/## (?:3\.\s*)?(?:세부 혜택 및 수치 비교|무엇이 얼마나 달라지나\?)/, `## 3. 각계 반응 및 주요 쟁점 비교`)
        .replace(/## (?:4\.\s*)?(?:신청 방법 및 향후 일정|어떻게 신청하나\?)/, `## 4. 향후 전망 및 관전 포인트`);
    }
  }
