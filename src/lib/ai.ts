...
[CRITICAL 3-2: 마크다운 비교표 작성 표준 규칙 (테이블 렌더링 깨짐 원천 차단)]
1. 마크다운 비교표의 헤더 행과 구분선 행(| :--- | :--- |)의 열(Column) 개수를 반드시 100% 정확하게 일치시키십시오. (헤더가 4열이면 구분선도 반드시 정확히 4열)
2. 테이블의 각 행은 반드시 명확한 줄바꿈(\\n)으로 구분하고, 절대 한 줄로 이어붙이지 마십시오.
3. 비교표 시작 전과 종료 후에는 반드시 빈 줄(\\n\\n)을 두어 일반 문단과 완전히 분리하십시오.
4. 표 바로 윗줄에 표 제목을 넣을 때는 반드시 '### 주요 비교' 처럼 H3 마크다운으로 표기하십시오.
5. 수치 범위 표기 시 취소선 문법과 충돌하지 않도록 깔끔하게 기술하십시오.
...
3. 본문(content): 기사 원문의 실제 성격(카테고리)을 먼저 파악한 후, 그에 맞는 맥락의 소제목을 생성하십시오:
   - 🏪 소상공인·자영업자 정책자금 / 특화 지원 사업 (소진공, 기업마당, 중기부 소상공인 공고):
     ## 1. 지원 사업 개요 및 목적 (300자 이상)
     ## 2. 지원 대상 및 선정 기준 (소상공인 요건) (350자 이상, 상시근로자수·매출액 등 소상공인 요건 상세 서술)
     ## 3. 지원 내용 및 한도 (금리, 지원금액 등) (350자 이상, 마크다운 비교표 | 지원 항목 | 일반 시중은행 조건 | 본 정책 우대 조건 | 세부 지원 내용 | 1개 이상 필수)
     ## 4. 신청 방법 및 구비 서류 (300자 이상, 소상공인정책자금 누리집 또는 기업마당 신청 절차)
     ## 5. 주의사항 및 접수 마감일 (300자 이상, 중복 수혜 배제 규정 및 조기 마감 유의사항)
   - 💰 정책·지원금 (일반 복지·청년·서민 보조금 사업):
   ...
   - [공통]: 공백 포함 1,300자 ~ 1,800자 이상 필수, 각 H2당 2~3문장 단위 문단 쪼개기(긴 벽돌글 절대 금지), H태그는 H1 없이 '##'(H2) 4~5개와 '###'(H3)으로만 구성.
...
소상공인 정책자금은 연간 배정된 정부 예산 범위 내에서 선착순으로 심사·지급되므로, 조기 마감에 유의하여 신속하게 접수해야 합니다.

### 신청 시 유의사항 및 접수 마감 체크포인트
- **예산 소진 시 조기 마감:** 자금별 배정 예산이 소진될 경우 당월 접수가 즉시 마감될 수 있으므로 매월 초 공고 개시 직후 신청 권장
- **불법 브로커 피해 주의:** 공단 및 지자체 직원을 사칭하여 수수료를 요구하거나 대출 승인을 보장한다는 사기 문자에 절대 응하지 마십시오.
- **중복 수혜 배제 확인:** 당해 연도 중기부 또는 지자체의 동일 유사 경영안정자금을 이미 지원받은 경우 한도 감액이나 지원 제한이 적용될 수 있습니다.
