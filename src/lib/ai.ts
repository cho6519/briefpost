...
[CRITICAL 3: 본문 H태그 위계 구조 (H-Tag Hierarchy Rule) - 구글 SEO & 애드센스 승인 절대 준수]
1. 본문 안에서 '#' (H1) 마크다운이나 '<h1>' 태그를 절대 사용하지 마십시오. (H1은 기사 메인 타이틀에 단 하나만 적용됩니다)
2. 본문의 핵심 섹션 제목은 반드시 마크다운 '##' (H2) 3개로 구성하십시오.
   - 정책·지원금 분야 권장 소제목 구조:
     - ## 1. 핵심 요약 및 개요
     - ## 2. 지원 자격 및 세부 요건
     - ## 3. 신청 방법 및 향후 일정
   - 기타 분야의 경우도 기사 핵심 흐름에 맞는 마크다운 '##' (H2) 소제목 3개로 균형 있게 작성하십시오.
3. 각 '##' (H2) 하위의 세부 항목이나 주의사항 타이틀은 반드시 '###' (H3)으로만 작성하십시오.
4. '####' (H4) 이하의 태그나 마크다운은 절대 사용하지 마십시오. (H태그는 반드시 H1(메인제목) ➔ H2(섹션) ➔ H3(세부항목)의 3단계로만 제한됩니다)
5. HTML 태그나 외부 링크는 본문에 절대 포함하지 마십시오.
...
### 세부 심사 기준 및 자격 검증
자격 요건 충족 여부는 관계 기관의 행정 정보 연계를 통해 검증되며, 추가 증빙 서류가 필요한 경우 기한 내 제출해야 합니다.
...
### 유의사항 및 접수 마감 안내
마감 기한 임박 시 접속 지연이 발생할 수 있으므로 가급적 조기에 접수를 완료할 것을 권장합니다.
    `.trim();
